...
void draw() {
  ...
  PImage resultBlurred = convolute(resultHue, blur, 99f);
  ...
  ArrayList<PVector> duples = hough(result, 4);
  
  //detects the intersections of the lines and add them to pg
  getIntersections(duples);
  //calcule quads
  ArrayList<int[]> quads = computeQuad(img, duples);
...
private ArrayList<int[]> computeQuad(PImage image, ArrayList<PVector> lines) {
  //create a quadGraph
  ...
  for (int[] quad : quads) {
    ...
    {
      resultsQuad.add(quad);
    }
  }
  println(resultsQuad.size()+" quads remaining out of " +quads.size() + " starting quads list");
  return resultsQuad;
}
...
PImage hueFilter(PImage img, float m, float d, float s, float b)
{
  ...
  for (int i = 0; i < img.width * img.height; i++) 
  {
    if (hue(img.pixels[i])<m+d&&hue(img.pixels[i])>m-d&&s<saturation(img.pixels[i])&&brightness(img.pixels[i])>b&&brightness(img.pixels[i])<255-b) {
      result.pixels[i]= color(255, 255, 255);
    } else {
      result.pixels[i]= color(0, 0, 0);
    }
